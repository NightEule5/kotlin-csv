...
actual class CsvWriter actual constructor(
        ...
        internal val ctx: CsvWriterContext
) : ICsvWriterContext by ctx {

    actual inline fun open(targetFileName: String, append: Boolean, write: ICsvFileWriter.() -> Unit) {
        val targetFile = File(targetFileName)
        open(targetFile, append, write)
    }

    actual suspend inline fun openAsync(targetFileName: String, append: Boolean, crossinline write: suspend ICsvFileWriter.() -> Unit) {
        val targetFile = File(targetFileName)
        openAsync(targetFile, append, write)
    }

    inline fun open(targetFile: File, append: Boolean = false, write: ICsvFileWriter.() -> Unit) {
        val fos = FileOutputStream(targetFile, append)
        open(fos, write)
    }

    suspend inline fun openAsync(targetFile: File, append: Boolean = false, crossinline write: suspend ICsvFileWriter.() -> Unit) = withContext(Dispatchers.IO) {
        val fos = FileOutputStream(targetFile, append)
        openAsync(fos, write)
    }

    inline fun open(ops: OutputStream, write: ICsvFileWriter.() -> Unit) {
        val osw = OutputStreamWriter(ops, ctx.charset)
        val writer = CsvFileWriter(ctx, PrintWriter(osw))
        writer.use { it.write() }
    }

    suspend inline fun openAsync(ops: OutputStream, crossinline write: suspend ICsvFileWriter.() -> Unit) = withContext(Dispatchers.IO) {
        val osw = OutputStreamWriter(ops, ctx.charset)
        val writer = CsvFileWriter(ctx, PrintWriter(osw))
        writer.use { it.write() }
    }
